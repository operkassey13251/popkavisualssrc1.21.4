    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder
                .then(literal("add")
                        .then(arg("player", word())
                                ...
                                .executes(context -> {
                                    ...
                                    if (!Popka.INSTANCE.staffStorage.isStaff(player)) {
                                        Popka.INSTANCE.staffStorage.add(player);
                ...
                .then(literal("remove")
                        .then(arg("player", word())
                                .suggests((context, builder1) -> {
                                    Popka.INSTANCE.staffStorage.getStaffs().stream()
                                ...
                                .executes(context -> {
                                    String player = context.getArgument("player", String.class);
                                    if (Popka.INSTANCE.staffStorage.isStaff(player)) {
                                        Popka.INSTANCE.staffStorage.remove(player);
                ...
                .then(literal("list")
                        .executes(context -> {
                            ...
                            if (Popka.INSTANCE.staffStorage.getStaffs().isEmpty()) {
                                ChatUtils.sendMessage("Список стаффов пуст!");
                            } else {
                                for (int i = 0; i < Popka.INSTANCE.staffStorage.getStaffs().size(); i++) {
                                    builder1.append(Popka.INSTANCE.staffStorage.getStaffs().get(i));
                                    if (i < Popka.INSTANCE.staffStorage.getStaffs().size() - 1) {
                                        builder1.append(", ");
                                    }
                                }
                                builder1.append(".");
                ...
                .then(literal("clear")
                        .executes(context -> {
                            if (!Popka.INSTANCE.staffStorage.isEmpty()) {
                                Popka.INSTANCE.staffStorage.clear();
