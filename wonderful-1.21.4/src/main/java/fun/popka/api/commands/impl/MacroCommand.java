    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder
                                ...
                                .then(arg("bind", word())
                                        ...
                                        .then(arg("command", greedyString())
                                                .executes(context -> {
                                                    ...
                                                    if (Popka.INSTANCE.macroStorage.getMacro(name) != null) {
                                                    ...
                                                    try {
                                                        ...
                                                        Popka.INSTANCE.macroStorage.add(new Macro(
                                                                name,
                                                                command,
                ...
                .then(literal("remove")
                        .then(arg("name", word())
                                .suggests((context, builder1) -> {
                                    Popka.INSTANCE.macroStorage.getNames().stream()
                                ...
                                .executes(context -> {
                                    String name = context.getArgument("name", String.class);
                                    if (Popka.INSTANCE.macroStorage.isEmpty()) {
                                        ChatUtils.sendMessage("Список макросов пуст!");
                                        return SINGLE_SUCCESS;
                                    }

                                    Macro macro = Popka.INSTANCE.macroStorage.getMacro(name);
                                    if (macro == null) {
                                        ChatUtils.sendMessage("Макрос " + name + " не найден!");
                                        return SINGLE_SUCCESS;
                                    }

                                    Popka.INSTANCE.macroStorage.remove(macro);
                ...
                .then(literal("list")
                        .executes(context -> {
                            ...
                            if (Popka.INSTANCE.macroStorage.getNames().isEmpty()) {
                                ChatUtils.sendMessage("Список макросов пуст!");
                            } else {
                                for (int i = 0; i < Popka.INSTANCE.macroStorage.getNames().size(); i++) {
                                    builder1.append(Popka.INSTANCE.macroStorage.getNames().get(i));
                                    if (i < Popka.INSTANCE.macroStorage.getNames().size() - 1) {
                                        builder1.append(", ");
                                    }
                                }
                                builder1.append(".");
                ...
                .then(literal("clear")
                        .executes(context -> {
                            if (!Popka.INSTANCE.macroStorage.isEmpty()) {
                                Popka.INSTANCE.macroStorage.clear();
