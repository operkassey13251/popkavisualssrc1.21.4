...
import fun.pizda.api.utils.namespaced.FileUtils;
import fun.pizda.client.modules.Module;
import fun.pizda.client.modules.impl.render.Interface;
import fun.pizda.client.modules.impl.render.base.InterfaceProcessing;
import fun.pizda.client.modules.impl.render.base.implement.TargetHud;
import fun.pizda.client.modules.impl.render.base.implement.WaterMark;
import fun.pizda.client.modules.settings.Setting;
import fun.pizda.client.modules.settings.implement.*;
...
public class ConfigStorage {
    ...
    public void saveConfig(String config) throws Exception {
        File file = new File(Pizda.INSTANCE.configsDir, config + extension);

        JsonObject object = new JsonObject();
        object.add("config", new JsonPrimitive(config));
        object.add("theme", new JsonPrimitive(Pizda.INSTANCE.themeStorage.getThemes().name()));
        object.add("language", new JsonPrimitive(Pizda.INSTANCE.localizationStorage.getLanguage().name()));
    ...
    public void loadConfig(String config) throws Exception {
        if (!FileUtils.exists(Pizda.INSTANCE.configsDir + "/" + config + extension)) return;
        JsonObject object;
        try (InputStream stream = Files.newInputStream(Paths.get(Pizda.INSTANCE.configsDir + "/" + config + extension));
        ...
        if (object.has("theme")) {
            ...
            for (ThemeStorage.Themes theme : ThemeStorage.Themes.values()) {
                if (theme.name().equals(themeName)) {
                    Pizda.INSTANCE.themeStorage.setThemes(theme);
                    break;
                }
            }
        }
        ...
        if (object.has("language")) {
            try {
                Pizda.INSTANCE.localizationStorage.setLanguage(LocalizationStorage.Language.valueOf(object.get("language").getAsString()));
    ...
    public void saveGlobals() throws Exception {
        File file = new File(Pizda.INSTANCE.globalsDir, "globals" + extension);
        JsonObject object = new JsonObject();
        ...
        Pizda.INSTANCE.friendStorage.getFriends().forEach(friendsArray::add);
        object.add("friends", friendsArray);

        JsonArray staffsArray = new JsonArray();
        Pizda.INSTANCE.staffStorage.getStaffs().forEach(staffsArray::add);
        object.add("staffs", staffsArray);

        JsonArray macrosArray = new JsonArray();
        Pizda.INSTANCE.macroStorage.getMacros().forEach(macro -> {
    ...
    public void loadGlobals() throws Exception {
        if (!FileUtils.exists(Pizda.INSTANCE.globalsDir + "/" + "globals" + extension)) return;
        JsonObject object;
        try (InputStream stream = Files.newInputStream(Paths.get(Pizda.INSTANCE.globalsDir + "/" + "globals" + extension));
             Reader reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
            object = JsonParser.parseReader(reader).getAsJsonObject();
        }
        ...
        if (object.has("friends")) {
            for (JsonElement element : object.get("friends").getAsJsonArray()) {
                if (Pizda.INSTANCE.friendStorage.isFriend(element.getAsString())) continue;
                Pizda.INSTANCE.friendStorage.add(element.getAsString());
            }
        }

        if (object.has("staffs")) {
            for (JsonElement element : object.get("staffs").getAsJsonArray()) {
                if (Pizda.INSTANCE.staffStorage.isStaff(element.getAsString())) continue;
                Pizda.INSTANCE.staffStorage.add(element.getAsString());
            }
        }
        ...
        if (object.has("macros")) {
            for (JsonElement element : object.get("macros").getAsJsonArray()) {
                try {
                    ...
                    if (name.isBlank() || Pizda.INSTANCE.macroStorage.getMacro(name) != null) {
                        continue;
                    }

                    Pizda.INSTANCE.macroStorage.add(new Macro(name, command, new BindSetting("bind", key)));
                } catch (Exception ignored) {
                }
            }
        }
