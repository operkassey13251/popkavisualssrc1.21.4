...
public class WaterMark extends InterfaceProcessing {
    ...
    public void DefaultStyle(EventRender.Default eventRender) {
        ...
        var statsIconFont = Fonts.getFont("pizda", 14);
        ...
        if (!Pizda.INSTANCE.themeStorage.getThemes().getTheme().getName().equals("Rainbow")) {
            iconTop = Pizda.INSTANCE.themeStorage.getThemes().getTheme().color[0];
        } else {
        ...
        float pizdaRectX = x;
        float pizdaRectY = y;
        float pizdaRectW = brandTextX + brandTextW + 1.5f - x;

        RenderUtils.drawDefaultHudThemedPanel(matrices, pizdaRectX, pizdaRectY, pizdaRectW, pizdaRectH, 2.8f, 3.3f, iconTop);
        if (drawSquares) {
        }
        ...
        float rect2X = pizdaRectX + pizdaRectW + 2.5f;
        ...
        RenderUtils.drawDefaultHudThemedPanel(matrices, rect2X, pizdaRectY, rect2W, rect2H, 2.8f, 3.3f, iconTop);
        if (drawSquares) {
            RenderUtils.drawHudSquarePattern(matrices, rect2X, pizdaRectY, rect2W, rect2H, iconTop);
        }
        ...
        float rectBtmY = pizdaRectY + pizdaRectH + 2f;
        ...
        float totalW = Math.max(pizdaRectW + 2f + rect2W, rectBtmW);
        draggable.setWidth(totalW);
        draggable.setHeight(showBottom ? (pizdaRectH + 1f + rectBtmH) : pizdaRectH);
    }
    ...
    private float getServerTps() {
        if (Pizda.INSTANCE == null || Pizda.INSTANCE.tpsCalc == null) {
            return 20.0f;
        }
        return Math.max(0.0f, Math.min(20.0f, Pizda.INSTANCE.tpsCalc.getTPS()));
    }
