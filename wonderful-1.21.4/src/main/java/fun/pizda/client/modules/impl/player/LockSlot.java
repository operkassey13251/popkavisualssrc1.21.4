...
import fun.pizda.mixin.SlotAccessor;
...
public class LockSlot extends Module {
    ...
    private int getHotbarSlotFromClick(int slotId) {
        ...
        int inventoryIndex = accessor.pizda$getIndex();
        if (accessor.pizda$getInventory() == mc.player.getInventory() && inventoryIndex >= 0 && inventoryIndex <= 8) {
